...
struct ExerciseBlockView: View {
    ...
    private func loadExerciseData() {
        ...
        ref.getDocument { document, error in
            ...
            let first = (document?.data()!["sets"] as? [[String: Any]])![0]
            
            if exercise.exerciseName == "Bird Dog" {
                let a = 7
            }
            
            if let fetchedSets = data["sets"] as? [[String: Any]] {
                self.sets = fetchedSets.compactMap { dict in
                    // ✅ Extract values safely without guard
                    ...
                    if exercise.exerciseName == "Bird Dog" {
                        print("Bird Dog set loaded:", setIndex, weight, reps, unit, week, day, dateCompleted)
                    }
...
//        let dateCompleted = dateFormatter.string(from: Date())
    ...
    private func buttonContent(for set: WorkoutSet, index: Int) -> some View {
        HStack {
            ...
            if sets.count > index {
                let currSet = sets[index]
                Text("\(currSet.reps ?? 0) | \(currSet.weight ?? 0, specifier: "%.1f") \(currSet.unit ?? "")")
